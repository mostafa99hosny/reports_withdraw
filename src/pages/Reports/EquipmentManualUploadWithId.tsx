...
const EquipmentManualUploadWithId: React.FC = () => {
  ...
  const handleSend = (id: string) => {
    setReports(prev => prev.map(r => (r.id === id ? { ...r, status: 'مرسلة' } : r)));
  };
    ...
    <div className="max-w-5xl mx-auto p-8 bg-white rounded-2xl shadow-lg" style={{ fontFamily: 'Cairo, Tahoma, Arial' }}>
      ...
      {step === 1 && (
        <div className="space-y-8">
          <div className="border-2 border-dashed border-gray-300 rounded-xl p-8 flex flex-col items-center text-center bg-white hover:border-blue-400 transition-colors">
            ...
            />
            <button type="button" className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-6 rounded-lg shadow transition-all cursor-pointer mt-2" onClick={() => document.getElementById('excel-upload-with-id')?.click()}>
              اختيار ملف Excel
            </button>
            {excelFile && (
      ...
      {step === 2 && (
        <div className="space-y-8">
          <div className="bg-gray-50 rounded-xl p-6 border">
            <label className="block mb-2 font-semibold text-gray-700" htmlFor="report-id-input">رقم التقرير</label>
            <input
              id="report-id-input"
              type="text"
      ...
      {step === 3 && (
        <div className="bg-white rounded-2xl shadow p-8 mb-8">
          <h2 className="text-2xl font-bold mb-6 text-green-700">ارسال التقارير</h2>
...
export default EquipmentManualUploadWithId;
